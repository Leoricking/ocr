...
- 完全不需要 Claude API 或 `CLAUDE_API_KEY`。
- `_OCR_raw.txt` 保留 PaddleOCR 原文。
- `_OCR_corrected.txt` 與 `_OCR.txt` 輸出本機詞庫、上下文及文件限定規則的校正版。
- 金融文件可修正「均綜→均線、日綜→日線、涸綜→週線、黃金交又→黃金交叉」等系統性錯字。
- 校正流程不發出網路請求。
...
- 財經雜誌雙欄閱讀順序自動修正
- 純離線金融／科學詞庫與上下文校正（不需要 Claude API）
- 固定式亂碼修正規則
...
1. 設定來源 PDF 或資料夾
2. 設定輸出資料夾
3. 調整 OCR 設定（裝置、Zoom、預處理模式、離線文字校正）
4. 點選「開始批次 OCR」
5. 可隨時暫停、繼續、取消
...
```powershell
python .\ocr_engine.py --input "D:\scan" --output "D:\output" --device gpu
```
...
## 離線文字校正（不需要 Claude API）

本版本已停用 Claude／Anthropic API。所有 OCR 後處理均在本機完成，不需要 API Key，也不會發出網路請求。

輸出檔案：

- `_OCR_raw.txt`：PaddleOCR 原始辨識結果。
- `_OCR_corrected.txt`：本機金融／科學詞庫、文件限定規則與上下文校正結果。
- `_OCR.txt`：與 corrected 相同，供既有流程使用。
- `_OCR_analysis.txt`：低信心與實際修正紀錄。

目前已加入《135均線技術分析》、《K線圖一看就懂》、《今周刊：摸透主力思維》、牛頓科學與《賴樹聲 電磁波》的安全文件限定規則。規則不使用單字全域替換，避免把「綜合」、「放量」、「一張圖」等正常文字誤改。

---

## v4.5.7 版面分區與區塊重跑

自動預處理模式現在會先做一次保守的整頁 OCR，再依文字幾何位置切分為標題、正文、圖說、圖表與雜訊區域。正文區塊會分別比較 ORIGINAL、LIGHT_CLAHE；品質低於 65 分時，再追加 CLAHE、BINARY 與局部放大重跑。

候選結果不再只依 Paddle confidence 選擇，還會評估中文可讀性、常用詞、金融術語、文件詞彙一致性、標點完整度，以及英數亂碼與單字碎片懲罰。圖表中的座標、狀態列與類似 `5230371MAW0L10` 的混合亂碼預設不會混入正文 TXT。

環境變數：

- `OCR_LAYOUT_AWARE=1`：啟用版面分區與區塊重跑，預設值。
- `OCR_LAYOUT_AWARE=0`：暫時回退到舊版整頁 OCR 流程。

所有流程均在本機執行，不需要 Claude API。
